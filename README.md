...
- `test:db-consistency`: 실제 DB에 임시 요청을 생성, 조회, 수정, 삭제하고 홈/요청 내역/상세 화면의 데이터 일관성을 확인합니다. 요청 내역이 날짜 묶음 없이 최신순 일반 리스트로 표시되고 오래된 요청도 같은 목록 페이지에서 확인되는지도 검증합니다. `app_settings.departments` 부서 DB 설정과 부서별 `기타` 카테고리도 확인합니다.
