...
- 현재 주가 비어 있으면 최근 요청이 있는 주 자동 표시
- 주차 페이지 이동
- 요청 상세 조회
- 부서/카테고리별 요청 품목 표시
- 관리자 체크 수정
...
- `test:db-consistency`: 실제 DB에 임시 요청을 생성, 조회, 수정, 삭제하고 홈/요청 내역/상세 화면의 데이터 일관성을 확인합니다. 월요일 요청이 주간 목록에서 빠지지 않는지, 현재 주가 비어 있을 때 최근 요청 주가 먼저 보이는지도 확인합니다. `app_settings.departments` 부서 DB 설정과 부서별 `기타` 카테고리도 확인합니다.
