...
| 먹자 | `m1234` | 요청 작성, 전체 요청 확인, 입고 확인, 메뉴/레시피 조회 |
...
- 카페테리아, 야채, 매장: 자기 부서로 들어온 최근 요청만 표시
- 먹자, 관리자: 전체 부서의 최근 요청 표시

요청 품목은 부서와 카테고리별로 묶여 표시됩니다. 홈 체크를 저장하면 관리자와 먹자 사용자는 입고 상태로, 그 외 사용자는 출고 상태로 요청 내역에 반영됩니다.

먹자와 관리자는 마지막 요청 제목 아래의 이동 컨트롤로 부서와 카테고리를 선택한 뒤 `이동`, `이전`, `다음`, `맨위`, `맨아래`로 긴 요청 목록을 이동할 수 있습니다. `맨위`는 마지막 요청 제목 영역으로 이동해 홈 상단 상태를 다시 확인하기 쉽게 합니다.

먹자와 관리자는 홈에서도 부서 제목 줄의 접기/펼치기 버튼으로 해당 부서의 카테고리를 한 번에 접거나 펼칠 수 있습니다. 카테고리 제목 줄의 접기/펼치기 버튼은 해당 카테고리 품목 목록만 접고 펼칩니다.
...
요청 내역은 최신 요청부터 일반 리스트로 표시됩니다.

![요청 내역 목록](docs/images/manual-history.png)

페이지 이동 버튼으로 이전/다음 목록을 볼 수 있습니다. 요청 내역은 날짜 묶음 없이 최신순으로 정렬하며, 한 페이지에는 최대 10개 요청이 표시됩니다. 목록에서 요청 행을 누르면 상세 화면이 열립니다.
...
- 저장된 요청이 없으면 빈 상태 메시지가 표시됩니다.
- 권한이 제한된 부서 사용자는 자기 부서 요청 중심으로 확인합니다.

## 5. 요청하기

요청하기 화면은 먹자와 관리자만 사용할 수 있습니다.
...
5. 요청 저장 후 `총 N개의 품목을 요청합니다.` 문구와 부서별 요청 메시지가 자동으로 생성됩니다.
6. 각 부서 메시지 아래의 `문자 복사 / 카톡 열기` 버튼을 누르면 해당 부서 메시지가 복사되고 카카오톡 앱이 열립니다.
...
요청 날짜는 사용하는 브라우저의 현지 날짜 기준으로 저장됩니다.

부서별 메시지는 요청하기 화면에 저장된 카테고리 순서와 품목 순서를 기준으로 생성됩니다. 카테고리별로 품목을 묶고, 마지막 줄에 `필요합니다`를 표시합니다. 총 요청 품목 수는 부서별 요청 메시지 섹션과 요청 버튼 사이에 표시됩니다.

`문자 복사 / 카톡 열기` 버튼은 메시지를 클립보드에 복사한 뒤 카카오톡 앱을 여는 실행 링크입니다. 자동으로 특정 채팅방에 메시지를 전송하지는 않습니다.
...
같은 부서 안에서는 같은 카테고리명을 생성하거나 수정할 수 없습니다. 같은 부서 안에서는 품목 한글명 또는 영문명이 이미 있는 경우에도 저장되지 않고 팝업으로 중복을 알립니다.

모든 부서에는 `기타` 카테고리가 유지됩니다. 카테고리를 삭제하면 해당 카테고리의 품목은 삭제되지 않고 같은 부서의 `기타`로 이동합니다. `기타` 카테고리는 삭제하거나 이름을 바꿀 수 없습니다.
...
카테고리 이동 패널의 `맨위` 버튼은 첫 카테고리가 아니라 요청하기 제목과 모드 버튼 영역으로 이동합니다. 긴 목록 아래쪽에서도 주문 모드/수정 모드 버튼으로 빠르게 돌아갈 수 있습니다.
...
3. 테스트 요청을 하나 저장합니다.
4. 요청 후 총 요청 품목 수, 부서별 메시지, `문자 복사 / 카톡 열기` 버튼이 보이는지 확인합니다.
5. 홈에 최신 요청이 보이고 관리자/먹자 이동 컨트롤과 접기/펼치기가 동작하는지 확인합니다.
6. 홈 체크 저장 후 관리자/먹자는 입고, 그 외 사용자는 출고 상태로 요청 내역 상세에 반영되는지 확인합니다.
