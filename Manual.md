...
| 레스토랑 | `m1234` | 요청 작성, 전체 요청 확인, 출고 확인, 메뉴/레시피 조회 |
...
레스토랑과 관리자는 마지막 요청 제목 아래의 이동 컨트롤로 부서와 카테고리를 선택한 뒤 `이동`, `이전`, `다음`, `맨위`, `맨아래`로 긴 요청 목록을 이동할 수 있습니다. `맨위`는 마지막 요청 제목 영역으로 이동해 홈 상단 상태를 다시 확인하기 쉽게 합니다.

레스토랑과 관리자는 홈에서도 부서 제목 줄의 접기/펼치기 버튼으로 해당 부서의 카테고리를 한 번에 접거나 펼칠 수 있습니다. 카테고리 제목 줄의 접기/펼치기 버튼은 해당 카테고리 품목 목록만 접고 펼칩니다.
...
5. 홈에 최신 요청이 보이고 관리자/레스토랑 이동 컨트롤과 접기/펼치기가 동작하는지 확인합니다.
