...
이 문서는 먹자 골목 관리 앱을 실제 사용자가 사용할 때 필요한 흐름을 설명합니다. 화면 캡처는 `docs/images`에 저장하며, 기능을 수정하면 기능 트리와 테스트 체크리스트도 함께 갱신합니다.
...
카테고리 제목 줄의 접기/펼치기 버튼으로 긴 품목 목록을 접을 수 있습니다. 접힌 카테고리는 카테고리명, 펼침 버튼, 이동 핸들만 표시되며, 카테고리명을 누르면 해당 위치로 포커스가 이동합니다.
...
10. 관리자 CSV Export/Import를 테스트합니다.
11. 이상이 없으면 DB 초기화와 운영 데이터 Import를 진행합니다.
