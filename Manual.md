...
요청 내역은 화요일부터 다음 주 월요일까지 7일 단위로 표시됩니다.

![요청 내역 목록](docs/images/manual-history.png)

주차 이동 버튼으로 이전/다음 묶음을 볼 수 있습니다. 요청 내역은 화요일부터 다음 주 월요일까지의 7일 단위로 묶입니다. 현재 주에 요청이 없고 이전 요청이 있으면 최근 요청이 있는 주를 1번 페이지로 먼저 보여줍니다. 목록에서 요청 행을 누르면 상세 화면이 열립니다.
...
- 현재 주가 비어 있어도 이전 요청이 있으면 최근 요청이 있는 주를 1번 페이지로 자동 표시합니다.
