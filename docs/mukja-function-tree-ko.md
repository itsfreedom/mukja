...
이 문서는 V1.0 기준 앱 전체 기능을 한 번에 보기 위한 기능 지도입니다. V1.0 이후 기능 개발은 새 브랜치에서 시작하고, 기능이 바뀌면 이 문서와 테스트 체크리스트를 함께 갱신합니다.
...
| 매장 | `g1234` | 자기 부서 요청/메모 | 자기 부서 요청 조회 | 사용 불가 | 사용 불가 | 조회 | 사용 불가 |
| 먹자 | `m1234` | 전체 요청/입고 체크/메모 | 전체 요청 조회 | 주문 모드 | 메뉴/레시피 조회 | 조회 | 사용 불가 |
| 레스토랑 | legacy | 읽기 전용 보조 역할 | 전체 요청 조회 | 사용 불가 | 사용 불가 | 조회 | 사용 불가 |
...
- 권한별 표시 범위 분기
- 일반 부서 사용자는 자기 부서 품목만 조회
- 먹자/관리자는 전체 부서 품목 조회
- 부서/카테고리별 품목 그룹 표시
- 먹자/관리자 이동 컨트롤
- 먹자/관리자 부서별 전체 접기/펼치기
- 먹자/관리자 카테고리 접기/펼치기
- 권한별 홈 체크 저장
- 관리자/먹자 사용자 입고 체크 저장
- 그 외 사용자 출고 체크 저장
...
- 메모만 있으면 요청 내역에 메모 요청으로 저장
- 취소 버튼으로 저장 전 변경 내용 취소
- 요청 저장
- 요청 저장 후 요청 내역 생성
...
- 메시지 아래 `문자 복사 / 카톡 열기` 버튼
- 부서별 요청 메시지 복사 후 카카오톡 앱 열기
...
- 재료 품목 삭제
- 재료 한글명/영문명 필수 입력 검증
- 같은 부서 안의 품목 한글명/영문명 중복 저장 방지
- 같은 부서 안의 카테고리명 중복 저장 방지
- 한글명/영문명 관리
- 부서 관리
- 카테고리 관리
- 카테고리 영문명 관리
...
- 가져오기 전 확인 창
- 가져오기 후 DB 교체
- 화면에 표시하지 않는 전체 CSV 백업 묶음 생성 함수
...
| 10 | 관리자 | 부서 CRUD, 비밀번호 CRUD, 데이터 백업 |
...
- [ ] 일반 부서 권한은 자기 부서 품목만 보인다.
- [ ] 먹자/관리자는 전체 부서 품목이 보인다.
- [ ] 요청 내역 DB의 최신 저장/수정 요청이 홈에 우선 표시된다.
- [ ] 관리자/먹자 홈에서 이동 컨트롤이 보인다.
- [ ] 일반 부서 권한 홈에서는 이동 컨트롤이 숨겨진다.
- [ ] 관리자/먹자 홈에서 부서 접기/펼치기가 동작한다.
- [ ] 관리자/먹자 홈에서 카테고리 접기/펼치기가 동작한다.
- [ ] 홈 이동 컨트롤의 이동 버튼은 선택 카테고리로 포커스를 옮긴다.
- [ ] 홈 이동 컨트롤의 `맨위` 버튼은 마지막 요청 제목 영역으로 이동한다.
- [ ] 관리자 홈 체크는 요청 내역의 입고 상태로 저장된다.
- [ ] 먹자 사용자 홈 체크는 요청 내역의 입고 상태로 저장된다.
- [ ] 그 외 사용자 홈 체크는 요청 내역의 출고 상태로 저장된다.
- [ ] 메모 저장/초기화가 동작한다.
...
- [ ] 최신순 일반 목록이 보인다.
- [ ] 오래된 요청도 날짜 묶음 없이 같은 일반 목록에서 확인된다.
- [ ] 페이지 이동이 동작한다.
...
- [ ] 품목 선택과 해제가 동작한다.
- [ ] 메모와 함께 요청 저장이 된다.
- [ ] 요청 날짜가 브라우저 현지 날짜 기준으로 저장된다.
- [ ] 품목과 메모가 모두 비어 있으면 요청 내역이 저장되지 않는다.
- [ ] 메모만 있으면 요청 내역에 메모 요청으로 저장되고 홈에서 메모만 보인다.
- [ ] 취소 버튼으로 마지막 저장 상태가 복구된다.
- [ ] 요청 저장 후 총 요청 품목 수가 표시된다.
- [ ] 요청 저장 후 부서별 메시지가 자동 생성된다.
- [ ] 메시지 품목은 화면 카테고리/품목 순서대로 보인다.
- [ ] 메시지는 카테고리별 라인으로 묶이고 `필요합니다`가 마지막 줄에 보인다.
- [ ] 각 메시지 아래 `문자 복사 / 카톡 열기` 버튼이 보인다.
- [ ] 버튼 클릭 시 부서별 메시지를 복사한 뒤 카카오톡 앱 열기를 시도한다.
...
- [ ] 같은 부서의 중복 카테고리명은 팝업 표시 후 저장되지 않는다.
- [ ] 같은 부서의 중복 품목 한글명은 팝업 표시 후 저장되지 않는다.
- [ ] 같은 부서의 중복 품목 영문명은 팝업 표시 후 저장되지 않는다.
- [ ] 일괄 부서/카테고리 변경이 된다.
- [ ] 모든 부서의 카테고리 목록에 `기타`가 있다.
- [ ] `기타` 카테고리는 삭제 버튼이 숨겨진다.
- [ ] `기타` 카테고리 이름 변경은 저장되지 않는다.
- [ ] 카테고리를 삭제하면 해당 품목은 삭제되지 않고 같은 부서 `기타`로 이동한다.
- [ ] 카테고리 추가/수정/삭제가 된다.
...
- [ ] `맨위` 버튼은 첫 카테고리가 아니라 요청하기 제목/모드 버튼 영역으로 이동한다.
...
- [ ] 관리자 섹션 순서가 부서 관리, 비밀번호 관리, 데이터 백업 순서다.
- [ ] 관리자 각 섹션 접기/펼치기가 동작한다.
- [ ] 사용자 추가가 된다.
- [ ] 사용자 수정이 된다.
- [ ] 사용자 삭제가 된다.
- [ ] 마지막 관리자 삭제가 막힌다.
- [ ] 부서 목록이 관리자 페이지에 표시된다.
- [ ] 부서 목록은 현재 언어의 부서명만 표시한다.
- [ ] 부서 추가/수정이 된다.
- [ ] 부서 역할 선택지가 부서 DB 기준으로 표시된다.
- [ ] 사용 중인 부서 삭제/비활성화가 막힌다.
- [ ] 숨겨진 전체 CSV 백업 묶음 함수가 동작한다.
...
1. 기능 트리, README, Manual, Error 문서를 V1.0 기준으로 맞춘다.
2. 사이드바에 `Version 1.0` 표시를 유지한다.
3. 새 기능 개발은 `main`에서 새 브랜치를 열어 진행한다.
4. V1.0 이후 기능 변경 시 이 기능 트리와 테스트 체크리스트를 함께 업데이트한다.
5. 한글 문서를 기준으로 추후 영어 문서를 별도 브랜치에서 작성한다.
